...
window.placeBet = async function(marketId, side, amount) {
    ...
    try {
        ...
        if (data.success) {
            // 1. Update Balance instantly in localStorage and re-render card
            ...
            // 2. Refresh the bets list from the /api/bets route
...
window.processWithdraw = async function() {
    ...
    try {
        ...
        const response = await apiFetch('withdraw', {
            method: 'POST',
            // Send as a number to match our backend Number(req.body.amount) fix
