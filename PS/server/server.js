  ...
  helmet({
     ...
     directives: {
        ...
        "style-src": ["'self'", "'unsafe-inline'", "https://cdnjs.cloudflare.com"],
                "connect-src": ["'self'", "https://ipapi.co", "https://*.ipapi.co", "https://polysoko.com", "https://polysoko.online", "https://api.weatherapi.com", "https://nominatim.openstreetmap.org", "ws:", "wss:"],
...
app.post('/api/admin/settle-backlog', authenticateAdmin, async (req, res) => {
    try {
        ...
        const footballToResolve = await dbAll(
            `SELECT id FROM markets WHERE category='football' AND settled=0 AND status='closed' AND (result IS NULL OR TRIM(result)='')`,
            []
        );
