...
function renderHistory() {
   ...
   const filtered = bets.filter(bet => {
    ...
    if (currentHistoryTab === "won")
        return ["won", "win"].includes(status);

    if (currentHistoryTab === "lost")
        return ["lost", "lose"].includes(status);

    if (currentHistoryTab === "cancelled")
    ...
    if (!filtered.length) {
        container.innerHTML = `<div class="empty-state">No ${currentHistoryTab} bets</div>`;
        return;
    }
...
async function fetchMarkets() {
    try {
        ...
        const markets = state.allMarkets || [];
        state.allMarkets = markets;
        state.filteredMarkets = state.filteredMarkets || markets;
        renderApp();
        throw err;
    }
}
 ...
 * TOAST
 */
function showToast(msg, color = "#1d9bf0") {
